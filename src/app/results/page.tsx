...
function Metric({ label, metrics, kind }: { label: string; metrics: Record<TeamCode, TeamMetrics>; kind: "longest" | "bigFive" }) {
  const value = (team: TeamCode) => kind === "longest" ? cm(metrics[team].longestCm) : cm(metrics[team].bigFiveTotalCm);
...
function BigFive({ metrics }: { metrics: Record<TeamCode, TeamMetrics> }) {
  return <div className="big-five-grid">{teams.map((team) => <div key={team}><b>{team} · fem längsta</b><div className="fish-chips">{metrics[team].topFiveLengths.map((length, index) => <span key={`${length}-${index}`}>{length.toFixed(0)} cm</span>)}{metrics[team].topFiveLengths.length === 0 && <span>Inga ännu</span>}</div></div>)}</div>;
}
