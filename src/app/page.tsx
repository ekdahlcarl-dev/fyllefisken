      ...
      <section className="hero">
        <div className="container hero-content">
          ...
          <p className="hero-copy">
            Samlingsplatsen för fisketävlingar, tveksamma rekord och livslång prestige bland kompisarna.
          </p>
          <div className="hero-actions">
            <a className="button button-primary" href="#competitions">Se nästa tävling</a>
            <a className="button button-secondary" href="#results">Se resultat</a>
          </div>
        </div>
      </section>
      ...
      <section className="section section-muted" id="results">
        <div className="container">
          ...
          <div className="table-wrap" role="region" aria-label="Aktuell resultatställning" tabIndex={0}>
            <table>
              <thead>
                <tr><th>Placering</th><th>Fiskare</th><th>Poäng</th><th>Största fisk</th><th>Vinster</th></tr>
              </thead>
              <tbody>
                {standings.map((row) => (
                  <tr key={row.name}>
                    <td>{row.place}</td><td>{row.name}</td><td>{row.points}</td><td>{row.biggest}</td><td>{row.wins}</td>
                  </tr>
                ))}
              </tbody>
            </table>
