...
import { calculateCompetitionScore, type ScoringCatch } from "./scoring.ts";

const c = (team: "MAJO" | "TORSK", dayNumber: 1 | 2 | 3, lengthCm: number): ScoringCatch => ({ team, dayNumber, lengthCm });

test("0–4 catches are summed entirely into Big Five", () => {
  const score = calculateCompetitionScore([c("MAJO", 1, 70), c("MAJO", 1, 80), c("MAJO", 1, 90)], false);
...
test("exactly five catches all contribute", () => {
  const score = calculateCompetitionScore([50, 60, 70, 80, 90].map((length) => c("MAJO", 1, length)), false);
  assert.equal(score.days[0].teams.MAJO.bigFiveTotalCm, 350);
});

test("more than five catches uses only the five longest", () => {
  const score = calculateCompetitionScore([40, 50, 60, 70, 80, 90, 100].map((length) => c("MAJO", 1, length)), false);
...
test("equal scoring categories split points 0.5 / 0.5", () => {
  const score = calculateCompetitionScore([c("MAJO", 1, 90), c("TORSK", 1, 90)], false);
...
test("daily Big Five never includes another day's catches", () => {
  const score = calculateCompetitionScore([c("MAJO", 1, 50), c("MAJO", 2, 150), c("TORSK", 1, 60)], false);
...
test("three-day Big Five uses five longest from combined pool", () => {
  const score = calculateCompetitionScore([c("MAJO", 1, 70), c("MAJO", 1, 90), c("MAJO", 2, 100), c("MAJO", 2, 80), c("MAJO", 3, 110), c("MAJO", 3, 60)], false);
...
test("all-catch totals never affect awarded points", () => {
  const score = calculateCompetitionScore([c("MAJO", 1, 100), c("MAJO", 1, 99), c("MAJO", 1, 98), c("MAJO", 1, 97), c("MAJO", 1, 96), ...Array.from({ length: 20 }, () => c("TORSK", 1, 10))], false);
...
test("results are independent of insertion order", () => {
  const catches = [c("MAJO", 1, 81), c("MAJO", 2, 91), c("TORSK", 1, 82), c("TORSK", 3, 92), c("MAJO", 3, 101), c("TORSK", 2, 99)];
  assert.deepEqual(calculateCompetitionScore(catches, true), calculateCompetitionScore([...catches].reverse(), true));
});

test("corrected catch is reflected immediately when recalculated", () => {
  assert.deepEqual(calculateCompetitionScore([c("MAJO", 1, 80), c("TORSK", 1, 90)], false).days[0].longestPoints, { MAJO: 0, TORSK: 1 });
  assert.deepEqual(calculateCompetitionScore([c("MAJO", 1, 100), c("TORSK", 1, 90)], false).days[0].longestPoints, { MAJO: 1, TORSK: 0 });
});

test("winner stays hidden until competition is complete", () => {
  assert.equal(calculateCompetitionScore([c("MAJO", 1, 100), c("TORSK", 1, 90)], false).winner, null);
});
