...
test("MAJO lead uses the required copy", () => {
  ...
  assert.equal(result.message, "MAJO leder med 2–1 (men alla vet att dom fuskar)");
});
...
test("digital season replaces historical row for the same year without double counting", () => {
  ...
  assert.deepEqual({ majo: result.majo, torsk: result.torsk }, { majo: 1, torsk: 2 });
});
...
test("digital true ties and missing winners count for neither team", () => {
  ...
  assert.deepEqual({ majo: result.majo, torsk: result.torsk }, { majo: 0, torsk: 0 });
});
